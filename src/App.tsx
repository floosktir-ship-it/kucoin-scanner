...
interface Signal {
  symbol: string; price: number; rsi: number; volume: number; signalIdx: number; chartData: any[]; timestamp: number;
}
    ...
    <div className="container">
      <header className="header">
        ...
        <div className="controls-wrapper">
          ...
          <div className="filter-group email-group">
            <input 
              type="email" 
            ...
            />
            <button className="join-btn" onClick={() => alert('Saved!')}>JOIN</button>
          </div>
      ...
      {loading ? <div className="loading-container"><div className="spinner"></div></div> : 
       filteredSignals.length === 0 ? <div className="empty-state"><h2>No Active Breakouts</h2></div> : (
        <div className="grid">
            ...
            <div key={sig.symbol} className="card" onClick={() => setSelectedSignal(sig)}>
              <div className="card-header">
                ...
                <div className="badges">
                  ...
                  <span className="badge time-val">{new Date(sig.timestamp).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</span>
                </div>
              </div>
        ...
        <div className="modal-overlay" onClick={() => setSelectedSignal(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <h2>{selectedSignal.symbol} Detail Analysis</h2>
              <div className="modal-stats">
                <span>Price: ${selectedSignal.price}</span>
                <span style={{color:'var(--warning)'}}>RSI: {selectedSignal.rsi.toFixed(2)}</span>
              </div>
            </div>
